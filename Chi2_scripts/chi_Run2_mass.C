...
void chi_Run2_mass() {
    ...
    for (size_t i = 0; i < scaleFactors.size(); ++i) {
        for (size_t j = 0; j < scaleFactors.size(); ++j) {
            ...
            std::string histName = "h3MassFlavorPairs_DATAMC_CHI_" + oss.str();
...
TH2D* chi2_hist_saved = (TH2D*)chi2_hist->Clone("chi2_hist_saved");
...
//c2->SetLogz();
chi2_hist->Draw("COLZ same");

// Define effective NDF value for the combined histogram
const double NDF = 30.0;  // 30 + 47 = 77

// Loop through all bins to divide the bin content by NDF
for (int i = 1; i <= chi2_hist->GetNbinsX(); ++i) {
    for (int j = 1; j <= chi2_hist->GetNbinsY(); ++j) {
        double zValue = chi2_hist->GetBinContent(i, j);
        chi2_hist->SetBinContent(i, j, zValue / NDF);  // Divide each bin content by NDF
        chi2_hist->SetBinError(i, j, sqrt(30) / NDF);  // Adjust error accordingly
    }
}

chi2_hist->GetZaxis()->SetTitle("#frac{#chi^{2}}{NDF}");

//chi2_hist2->GetZaxis()->SetMoreLogLabels();
//chi2_hist2->GetZaxis()->SetNoExponent();
h->GetXaxis()->SetLabelSize(0.045);
h->GetYaxis()->SetLabelSize(0.045);
chi2_hist->GetZaxis()->SetLabelSize(0.045);
h->GetXaxis()->SetTitleSize(0.045);
h->GetYaxis()->SetTitleSize(0.045);
chi2_hist->GetZaxis()->SetTitleSize(0.045);

h->GetXaxis()->SetTitleOffset(1.1);
h->GetYaxis()->SetTitleOffset(1.4);
chi2_hist->GetZaxis()->SetTitleOffset(1.1);
...
cout << "chi2_hist->GetMinimum(): " << chi2_hist->GetMinimum() << endl << flush;
...
// Save the canvas
c2->SaveAs("pdf/chi_Run2_mass_chi_FSR_after.pdf");


// Save the chi2_hist to a ROOT file
TFile* outputFile = new TFile("chi2_hist_run2_mass_chi_FSR_after.root", "RECREATE");
chi2_hist_saved->Write();
g2->Write();
outputFile->Close();
