...
void allratio_ISR_psweights() {
    ...
    TFile* outputFile = new TFile("processed_histograms/histograms_ISR_psweights.root", "RECREATE");
    ...
    std::vector<std::string> suffixes = {"isr:murfac=0.25", "isr:murfac=0.5", "isr:murfac=2.0", "isr:murfac=4.0"};
    std::vector<std::string> legendNames = {"ISR (0.25)", "ISR (0.5)", "ISR (2.0)", "ISR (4.0)"};
    ...
    std::vector<std::string> extraNames       = {"h3all_ISR070", "h3all_ISR130"};
    std::vector<std::string> extraLegendNames = {"ISR (0.70)", "ISR (1.30)"};
    ...
    c2->SaveAs("pdf/all_ratio_ISR_psweights.pdf");
