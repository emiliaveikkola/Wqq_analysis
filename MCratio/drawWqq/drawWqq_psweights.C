...
void drawWqq_psweights() {
  ...
  TFile *ffsr = new TFile("processed_histograms/histograms_FSR_psweights.root","READ");
  assert(ffsr && !ffsr->IsZombie());

  TFile *fjer = new TFile("processed_histograms/histograms_JERsmear_fit.root","READ");
  assert(fjer && !fjer->IsZombie());

  TFile *fjer2 = new TFile("processed_histograms/histograms_JER.root","READ");
  assert(fjer2 && !fjer2->IsZombie());

  curdir->cd();
  
  // 40,160 or 70,130
  hisr_dw = (TH1D*)fisr->Get("RatioHist_isr:murfac=0.25"); assert(hisr_dw);
  hisr_up = (TH1D*)fisr->Get("RatioHist_isr:murfac=4.0"); assert(hisr_up);

  // 0990,1010 or 0995,1005 
  hfsr_dw = (TH1D*)ffsr->Get("RatioHist_fsr:murfac=0.5"); assert(hfsr_dw);
  hfsr_up = (TH1D*)ffsr->Get("RatioHist_fsr:murfac=2.0"); assert(hfsr_up);

  // 80,120 or 90,110
  hjer_dw = (TH1D*)fjer2->Get("RatioHist_JER090"); assert(hjer_dw);
  hjer_up = (TH1D*)fjer2->Get("RatioHist_JER110"); assert(hjer_up);

  TH1D *hdata = (TH1D*)fjer->Get("RatioHist_Data"); assert(hdata);
  ...
  TH1D *h1 = tdrHist("h1","Ratio to MC",0.76,1.28,"Mass (GeV)",66.,97.);
  //lumi_13TeV = "Run2";
  extraText = "Private";
  TCanvas *c1 = tdrCanvas("c1",h1,4,8,kSquare);

  TH1D *h2 = tdrHist("h2","Data/fit -1 (%)",-6,5,"Mass (GeV)",66.,97.);
  ...
  /*
  tdrDraw(hisr_up,"Pz",kOpenSquare,kBlue-9);
  tdrDraw(hisr_dw,"Pz",kFullSquare,kBlue);

  tdrDraw(hfsr_up,"Pz",kOpenCircle,kRed-9);
  tdrDraw(hfsr_dw,"Pz",kFullCircle,kRed);

  tdrDraw(hjer_up,"Pz",kFullDiamond,kGreen+2);
  tdrDraw(hjer_dw,"Pz",kOpenDiamond,kGreen-9);

  tdrDraw(hdata,"Pz",kFullCircle,kBlack);
*/
  //TF1 *fit = new TF1("fit",fit_func,66,97,3);
  TF1 *fit1 = new TF1("fit",fit_func,70,100,3); // use co-opted range 97-100 GeV
  ...
  latex.DrawLatex(0.32, 0.80, Form("ISR: %.3f #pm %.3f", par0, err0));
  latex.DrawLatex(0.32, 0.75, Form("FSR: %.3f #pm %.3f", par1, err1));
  latex.DrawLatex(0.32, 0.70, Form("JER: %.3f #pm %.3f", par2, err2));


  // Add the legend after all drawings
  TLegend *legend = tdrLeg(0.65, 0.88-0.04*8, 0.95, 0.88); 

  legend->AddEntry(hisr_up_scaled, "ISR (4.0)", "P");
  legend->AddEntry(hisr_dw_scaled, "ISR (0.25)", "P");
  legend->AddEntry(hfsr_up_scaled, "FSR (2.0)", "P");
  legend->AddEntry(hfsr_dw_scaled, "FSR (0.5)", "P");
  legend->AddEntry(hjer_up_scaled, "JER (1.10)", "P");
  legend->AddEntry(hjer_dw_scaled, "JER (0.90)", "P");
  legend->AddEntry(hdata_scaled, "Data", "P");
  ...
  TH1D *h_fit = (TH1D*)hdata->Clone("h_fit");
...
// Transform the ratio histogram to (data/fit - 1) * 100
  ...
  h_ratio->SetLineColor(kBlack);
   ...
   // Save the canvas
  c->SaveAs("pdf/drawWqq_psweights.pdf");   // Save as PDF
