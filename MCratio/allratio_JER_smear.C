...
double gFunction(double *x, double *params) {
    ...
    double zero_factor = (shifted_x - 76) * (shifted_x - 86) * (shifted_x - 100); // Ensures zeros at 76, 82, 100
    double h_x = params[0] * pow(shifted_x, 2) + params[1] * shifted_x + params[2]; // Quadratic modifier
    return zero_factor * h_x;
*/
return 1;
}
...
void allratio_JER_smear() {
    ...
    TFile* outputFile = new TFile("processed_histograms/histograms_JERsmear_fit.root", "RECREATE");
    ...
    std::vector<std::string> suffixes = {"smear"};
    std::vector<std::string> legendNames = {"JER (smear 1.10)"};
    ...
    //TCanvas* c2 = new TCanvas("c2", "Single Ratio Histogram", 600, 600);
    ...
    tex.DrawLatex(0.31, 0.85, "p_{T} > 30 GeV");
    tex.DrawLatex(0.31, 0.8, "|#eta| < 2.5");
    ...
    TLegend* ratioLegend = tdrLeg(0.49, 0.75, 0.93, 0.9);
    ...
    // --- BEGIN: Configurable extra histogram block ---
    // Define names, legend labels, colors, and markers for extra histograms
    std::vector<std::string> extraNames        = {"h3all_JER090", "h3all_JER110"};
    std::vector<std::string> extraLegendNames  = {"JER (0.90)", "JER (1.10)"};
    ...
    if (file && !file->IsZombie()) {
        for (size_t k = 0; k < extraNames.size(); ++k) {
            TH1D* hist_extra = (TH1D*)file->Get(extraNames[k].c_str());
            if (hist_extra) {
                ...
                if (extraNames[k] == "h3all_JER090" || extraNames[k] == "h3all_JER2090") {
                    ratioHist_extra->SetMarkerSize(1.5);  // Specific range for "h3tagxall"
                }
                if (extraNames[k] == "h3all_JER110" || extraNames[k] == "h3all_JER2110") {
    ...
    // --- END: Configurable extra histogram block ---

    ratioLegend->Draw();

    // Save the single ratio histogram as a PDF
    c2->SaveAs("JER/JERsmeared2.pdf");
